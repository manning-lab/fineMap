...
task preprocess {
	...
	command {
		echo ${interval}
		R --vanilla --args ${sep=":" interval_array} ${gds_file} ${sep="," sample_ids} ${sep="," assoc_files} ${annotation_file} ${anno_cols} ${default="10" mac} ${pval_col} ${effect_col} ${default="0.01" pval_thresh} < /fineMap/paintor/preprocess.R
	}
...
workflow group_assoc_wf {
	...
	scatter( this_interval_pair in these_interval_pairs ) {
		...
		call preprocess {
			input: interval_array = this_interval_pair.left, interval_string = this_interval_pair.right, gds_file = these_gds[this_chr_int], sample_ids = these_sample_ids, assoc_files = these_assoc, annotation_file = this_annotation_file, anno_cols = these_anno_cols, mac = this_mac, pval_col = this_pval_col, effect_col = this_effect_col, pval_thresh = this_pval_thresh, memory = pre_memory, disk = this_disk
		}
